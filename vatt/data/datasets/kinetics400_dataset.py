...
class KineticFactory(loading.BaseDMVRFactory, abc.ABC):
  """Base class for a toy factory."""

  _BASE_DIR = 'vatt/data/datasets'
  _NUM_CLASSES = 400
  ...
  def __init__(self, subset = 'train', split = 1):
    ...
    del split
    if subset not in KineticFactory._TABLES:
      raise ValueError('Invalid subset "{}". The available subsets are: {}'
                       .format(subset, KineticFactory._TABLES.keys()))

    super().__init__(KineticFactory._BASE_DIR,
                     table=f'{subset}',
                     source='tfrecord',
